...
export const MahjongGameTab: React.FC<MahjongGameTabProps> = ({
  ...
  const gameModeRef = useRef<'solo' | 'multiplayer'>('solo');
  ...
  // 同步外部 rules (仅单机模式下跟随外部修改，联机时遵从房主规则)
  useEffect(() => {
  ...
  const broadcastSync = useCallback((
    ...
    if (gameModeRef.current !== 'multiplayer' || !multiplayerService.isHost) return;
  ...
  // --------------------------------------------------------------------------
  // 客端同步房主开局权威牌桌状态 (不独立洗牌，完全采用房主发牌)
  // --------------------------------------------------------------------------
  const initGuestGameFromHost = useCallback((payload: any) => {
  ...
  const startNewGame = (
    ...
    // 房主在联机模式下广播权威开局发牌与状态给客端
    if (currentMode === 'multiplayer' && multiplayerService.isHost) {
      multiplayerService.startGame({
        players: newPlayers,
        wallCount: replaced.wall.length,
    ...
    // 若庄家是 AI，触发 AI 出牌 (仅单机或联机房主执行 AI)
    if (dealerPlayer.isAI && (currentMode !== 'multiplayer' || multiplayerService.isHost)) {
  ...
  const handleDiscardTile = (playerIndex: GameSeatIndex, tile: GameTile) => {
    // 联机模式下客端不执行本地权威出牌，由房主统一裁决
    if (gameModeRef.current === 'multiplayer' && !multiplayerService.isHost) {
      return;
    }
  ...
  const advanceToNextTurn = (nextSeat: GameSeatIndex) => {
    // 联机模式下客端不推进本地回合或摸牌，等待房主 GAME_STATE_SYNC 权威同步
    if (gameModeRef.current === 'multiplayer' && !multiplayerService.isHost) {
      return;
    }
    ...
    let drawn: GameTile | undefined = currentWall.pop();
    if (!drawn) {
      handleDrawGame();
      return;
    }
  ...
  const executeKong = (seat: GameSeatIndex, tile: GameTile, type: 'ming' | 'an' | 'bu') => {
    ...
    let currentFlowers = [...konger.flowers];

    let replacementTile: GameTile | undefined = currentWall.pop();

    // 杠牌补牌：若补到的牌是花牌/动物/小丑，自动收入花牌区，并从牌墙继续摸补牌，直到摸到正牌或牌墙摸空
    while (replacementTile && isBonusTile(replacementTile)) {
      currentFlowers.push(replacementTile);
      soundFx.playTileClick();
      if (currentWall.length === 0) {
        replacementTile = undefined;
        break;
      }
      replacementTile = currentWall.pop();
    }
    ...
    // 杠上开花判断
    const kongBloomCheck = checkCanPlayerWin(
      sortedHand,
      currentPlayers[seat].melds,
      currentFlowers,
      undefined,
      true,
      konger.wind,
    ...
    if (kongBloomCheck.canWin && kongBloomCheck.calcResult) {
      if (seat === mySeatIndex) {
        setBannerMsg('🎉 恭喜！杠上开花自摸达成，可点击【自摸胡】！');
      } else if (konger.isAI && (gameMode !== 'multiplayer' || multiplayerService.isHost)) {
        setTimeout(() => {
          handleDeclareWin(seat, 'zimo', undefined, kongBloomCheck.calcResult!);
        }, 800);
        return;
      }
    }
  ...
  // --------------------------------------------------------------------------
  // 大厅启动对战统一入口 (客端直接接入房主牌桌，房主生成权威洗牌)
  // --------------------------------------------------------------------------
  const handleStartGame = useCallback((
    mode: 'solo' | 'multiplayer',
    roomPlayers: { name: string; isAI: boolean; seat: GameSeatIndex }[],
    hostRules: RuleSettings,
    initialPayload?: any
  ) => {
  ...
  const handleSortMyHand = () => {
    ...
    setPlayers(prev => {
      const copy = [...prev];
      const me = copy[mySeatIndex];
      const regular: GameTile[] = [];
      const strayFlowers: GameTile[] = [];
      ...
      if (strayFlowers.length > 0) {
        wallModified = true;
        const newFlowers = [...me.flowers, ...strayFlowers];
        const drawnReplacements: GameTile[] = [];

        for (let i = 0; i < strayFlowers.length && currentWall.length > 0; i++) {
          let rep: GameTile | undefined = currentWall.pop();
          while (rep && isBonusTile(rep)) {
            newFlowers.push(rep);
            if (currentWall.length === 0) {
              rep = undefined;
              break;
          ...
          if (rep) {
            drawnReplacements.push(rep);
          }
        }

        const sortedHand = sortHandTiles([...regular, ...drawnReplacements]);
        copy[mySeatIndex] = {
          ...me,
          hand: sortedHand,
          handCount: sortedHand.length,
        ...
        setBannerMsg(`🌸 理牌完成：已自动将 ${strayFlowers.length} 张花牌归位并摸牌补花！`);
      } else {
        copy[mySeatIndex] = {
          ...me,
          hand: sortHandTiles(me.hand),
        };
      }
  ...
  // --------------------------------------------------------------------------
  // 监听联机消息协议 (客端与房主) - 使用 ref 避免 rules 变化导致重订阅
  // --------------------------------------------------------------------------
  const activeGameRulesRef = useRef(activeGameRules);
  activeGameRulesRef.current = activeGameRules;
  ...
  useEffect(() => {
    ...
    const unsubStateSync = multiplayerService.on('gameStateSync', (payload: any) => {
      if (payload.wallCount !== undefined) {
        const dummyWall = new Array(payload.wallCount).fill(null) as any;
        wallRef.current = dummyWall;
        setWall(dummyWall);
      ...
      if (payload.players) {
        // 客端收到同步后，确保自身手牌保持整齐排序，且手牌绝不滞留花牌
        ...
        const syncedPlayers: GamePlayer[] = payload.players.map((p: GamePlayer, idx: number) => {
          ...
          const cleanHand = idx === mySeat ? sortHandTiles(regular) : regular;
          return {
            ...p,
            hand: cleanHand,
            handCount: cleanHand.length,
            flowers: strayFlowers.length > 0 ? [...p.flowers, ...strayFlowers] : p.flowers,
          };
        });
        playersRef.current = syncedPlayers;
        setPlayers(syncedPlayers);
    ...
    // 房主接收客端操作
    const unsubPlayerAction = multiplayerService.on('playerAction', (action: any) => {
      if (action.actionType === 'PLAYER_DISCARD' && action.payload?.tile) {
        ...
        } else if (claim === 'selfKong' && tile) {
          executeKong(action.seat, tile, action.payload?.kongType || 'an');
        } else if (claim === 'win') {
    ...
    <div className="flex flex-col space-y-3 sm:space-y-4 max-w-5xl mx-auto w-full">
      ...
      <div className="bg-[#0b331f]/90 border border-emerald-800/80 rounded-2xl p-3 sm:p-4 shadow-xl flex flex-wrap items-center justify-between gap-2.5 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* 重新发牌 / 新局 (单机或联机房主可用) */}
          {(gameMode !== 'multiplayer' || multiplayerService.isHost) && (
            <button
              type="button"
              ...
              {mySelfKongCheck.canKong && mySelfKongCheck.targetTileId && (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (tile) {
                      if (gameModeRef.current === 'multiplayer' && !multiplayerService.isHost) {
                        multiplayerService.sendToHost('PLAYER_CLAIM', {
                          claimAction: 'selfKong',
                          tile,
                          kongType: mySelfKongCheck.type || 'an',
                        });
                      } else {
                        executeKong(mySeatIndex, tile, mySelfKongCheck.type || 'an');
                      }
                    }
                  }}
          ...
          <div className="relative w-full max-w-lg bg-gradient-to-b from-[#0d3622] to-[#061910] border border-amber-500/60 rounded-3xl shadow-2xl overflow-hidden text-slate-100 flex flex-col max-h-[90vh]">
            ...
            <div className="p-5 space-y-4 overflow-y-auto flex-1 text-xs">
              ...
              <div className="flex gap-2 pt-2">
                ...
                {gameMode === 'multiplayer' && !multiplayerService.isHost ? (
                  <div className="flex-1 py-3 px-4 rounded-xl bg-emerald-950/80 border border-emerald-700 text-center text-xs text-amber-300 animate-pulse font-bold flex items-center justify-center gap-1.5">
                    <span>⏳ 等待房主开启下一局...</span>
                  </div>
                ) : (
                  <button
